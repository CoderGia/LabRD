...
def preemphasis(signal, pre_emphasis=0.97):
    ...
    """
    emphasized_signal = np.zeros(signal.shape)
    ###########################################################
    for i in range(len(signal)):
        if i == 0:
            emphasized_signal[i] = signal[i]
        else:
...
def framing(emphasized_signal, sample_rate=16000, frame_size=0.025, frame_stride=0.01):
    ...
    frames = np.array([np.array(pad_signal[i:i + frame_length] * hamming_window) for i in
                       range(0, len(pad_signal) - frame_length, frame_step)])
    ###########################################################
...
def compute_fbank_filters(nfilt=40, sample_rate=16000, NFFT=512):
    ...
    high_freq_mel = 1125 * log(1 + high_freq / 700)
    ###########################################################
